...
var replaceTopicReferenceWithCustomSlot = function (model, topicReference) {
  var topicReferenceToken = '{' + topicReference + '}';
  var valueToken = '{@' + topicReference + '}';
  return model.replace(topicReferenceToken, valueToken);
};
...
var topics = {
  ...
  airport: {
    ...
    count: 100,
    entries: [
      { id: 'dfw', values: ['dfw', 'dallas fort worth'] },
      { id: 'sea', values: ['sea', 'seatac airport', 'seatac'] }
    ]
  },
  city: {
    type: 'dictionary',
...
var model = 'Find flights from {@airport} on {@airline} in {@city} at {n}';
var model = '{n}';
var results = compile(model, topics);
